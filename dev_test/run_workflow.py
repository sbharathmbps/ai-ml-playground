...
class TokenProvider:
    """
    Reads & caches a projected SA token from file.
    Refreshes when file mtime changes or when forced (e.g., after 401/403).
    Optionally checks token exp to decide proactive refresh.
    """
    def __init__(self, token_path: str):
        self.token_path = token_path
        self._cached_token: Optional[str] = None
        self._cached_mtime: Optional[float] = None
        self._cached_exp: Optional[int] = None  # epoch seconds

    def _read_file(self) -> str:
        # Robust read; retry briefly if file is being rotated
        for _ in range(3):
            try:
                with open(self.token_path, "r") as f:
                    data = f.read().strip()
                if data:  # avoid zero-length reads
                    return data
            except Exception:
                pass
            time.sleep(0.05)
        raise RuntimeError(f"Failed to read token from {self.token_path}")

    def _decode_exp(self, token: str) -> Optional[int]:
        try:
            # Decode without verifying signature; we only want 'exp'
    ...
    def _reload(self) -> None:
        ...
        self._cached_exp = self._decode_exp(token)
    ...
    def get(self) -> str:
        ...
        # Refresh when file changed
        try:
            mtime = os.stat(self.token_path).st_mtime
            if self._cached_mtime != mtime:
                self._reload()
        except FileNotFoundError:
            # If the path temporarily disappears during rotation, force reload
            self._reload()
        # Optional proactive refresh if near expiry (e.g., < 2 minutes left)
        if self._cached_exp is not None and (self._cached_exp - time.time() < 120):
            self._reload()
        return self._cached_token
